 ...
 *
 * This file implements platform wrapper for the GPR datalink layer using OpenAmp.
 *
...
#error "GPR openamp datalink requires definition of shared memory for rpmsg"
#endif
...
/* OpenAMP management task stack size for GPR datalink */
#define GPR_OPENAMP_MNG_STACK_SIZE (1024)

/* GPR RX task thread stack size */
#define GPR_RX_TASK_STACK_SIZE (1536)

K_THREAD_STACK_DEFINE(thread_mng_stack, GPR_OPENAMP_MNG_STACK_SIZE);
K_THREAD_STACK_DEFINE(gpr_rx_thread_stack, GPR_RX_TASK_STACK_SIZE);

static struct k_thread thread_mng_data;
static struct k_thread gpr_rx_thread_data;
...
static struct rpmsg_endpoint gpr_ept;
static struct rpmsg_rcv_msg gpr_msg;

static K_SEM_DEFINE(data_sem, 0, 32);
static K_SEM_DEFINE(gpr_rx_sem, 0, 1);

#define MAX_QUEUED_MESSAGES 32
K_MSGQ_DEFINE(rpmsg_msgq, sizeof(struct rpmsg_rcv_msg), MAX_QUEUED_MESSAGES, 4);

#define RPMSG_ENDPOINT_NAME "audioreach-rpmsg"
...
static int gpr_rpmsg_recv_callback(struct rpmsg_endpoint *ept, void *data,
                   size_t len, uint32_t src, void *priv)
{
    ...
    k_sem_give(&gpr_rx_sem);

    return RPMSG_SUCCESS;
}

void gpr_rpmsg_rx_task(void *arg1, void *arg2, void *arg3)
{
    ARG_UNUSED(arg2);
    ARG_UNUSED(arg3);

    gpr_dl_zephyr_port_t *dl_zephyr_port = (gpr_dl_zephyr_port_t *)arg1;
    int ret = 0;
    uint32_t status = 0;

    k_sem_take(&gpr_rx_sem,  K_FOREVER);

    AR_MSG(DBG_LOW_PRIO, "GPR RX task started");

    gpr_ept.priv = &gpr_msg;
    ret = rpmsg_create_ept(&gpr_ept, rpdev, RPMSG_ENDPOINT_NAME,
                   RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
                   gpr_rpmsg_recv_callback, NULL);
    if (ret) {
        AR_MSG(DBG_ERROR_PRIO, "Could not create endpoint: %d", ret);
        goto task_end;
    }

    AR_MSG(DBG_LOW_PRIO,"GPR endpoint created successfully with rc: %d", ret);

    struct rpmsg_rcv_msg msg;
    while (gpr_ept.addr !=  RPMSG_ADDR_ANY) {
        ...
        while (k_msgq_get(&rpmsg_msgq, &msg, K_NO_WAIT) == 0) {
            if (msg.len) {
                ...
                rpmsg_release_rx_buffer(&gpr_ept, msg.data);
            }
            msg.len = 0;
            msg.data = NULL;
        }
    ...
    AR_MSG(DBG_LOW_PRIO, "GPR RX task: connection disconnected");

    rpmsg_destroy_ept(&gpr_ept);

task_end:
    AR_MSG(DBG_LOW_PRIO, "GPR RX task ended");
}
                        ...
                        ipc_to_gpr_vtbl_t ** pp_ipc_to_gpr_vtbl)
{
    ...
    *pp_ipc_to_gpr_vtbl = &gpr_dl_zephyr_vtbl;
    ...
    k_thread_create(&thread_mng_data, thread_mng_stack, GPR_OPENAMP_MNG_STACK_SIZE,
        rpmsg_mng_task,
        NULL, NULL, NULL, K_PRIO_COOP(8), 0, K_NO_WAIT);

    AR_MSG(DBG_LOW_PRIO, "rpmsg_mng_task thread launched");

    k_thread_create(&gpr_rx_thread_data, gpr_rx_thread_stack, GPR_RX_TASK_STACK_SIZE,
        gpr_rpmsg_rx_task,
        dl_zephyr_port, NULL, NULL, K_PRIO_COOP(7), 0, K_NO_WAIT);

    AR_MSG(DBG_LOW_PRIO, "GPR RX task thread launched");
...
uint32_t ipc_dl_zephyr_deinit(uint32_t src_domain_id, uint32_t dest_domain_id)
{
    ...
    /* ToDo: Handle thread termination for rpmsg_mng_task and gpr_rpmsg_rx_task */
...
static uint32_t gpr_dl_zephyr_send(uint32_t domain_id, void *buf, uint32_t size)
{
    ...
    status = rpmsg_send(&gpr_ept, tx_buff, size);
    if (status < 0) {
        AR_MSG(DBG_ERROR_PRIO, "rpmsg_send failed with status %d", status);
        if (status == -ENETRESET)
            return AR_ESUBSYSRESET;
        else
            return AR_EFAILED;
    }
    if (dl_zephyr_port->send_done)
