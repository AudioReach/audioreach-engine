...
capi_err_t capi_nxp_device_dai_init(capi_nxp_device_t *me_ptr, uint32_t dir)
{
   ...
   cfg.dai_index = 3;
   //cfg.format = 1;
   cfg.format = DAI_CBC_CFC | DAI_PROTO_I2S;
   cfg.options = 1;
   cfg.rate = 48000;
...
capi_err_t capi_nxp_device_set_dma_config(capi_nxp_device_t *me_ptr, uint32_t period_bytes, uint32_t period_count)
{
   ...
   /* 
      burst = SAI FIFO watermark × word_size
      watermark is hardcoded to 64 in SDMA_PrepareTransfer() call
      .source_burst_length = 4,
      .dest_burst_length   = 4,
   */
   dma_cfg->channel_direction = MEMORY_TO_PERIPHERAL;
   dma_cfg->source_data_size = 2;   /* 16-bit FIFO element */
